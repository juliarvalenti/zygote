...
pauses together; `live` keeps it moving), and `output` (`color`, `depth`
as gray, or `color_depth_alpha`, which puts depth in alpha so `luma_mask`
...
| `color_depth_alpha` keyed through `luma_mask` on alpha | `output: depth` |
| --- | --- |
